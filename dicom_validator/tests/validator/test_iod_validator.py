...
class IODValidatorTest:
    ...
    def has_tag_error(messages, module_name, tag_id_string, error_kind):
        ...
        for message in messages[module_name]:
            if message.startswith(
                    f'Tag {tag_id_string} is {error_kind}'):
                return True
        return False
    ...
    def test_fulfilled_condition_existing_tag(self, validator):
        ...
        assert not self.has_tag_error(result, 'Enhanced X-Ray Angiographic Image',
                                      '(0020,0052)', 'missing')
        assert not self.has_tag_error(result, 'Synchronization',
                                      '(0018,106A)', 'missing')
    ...
    def test_fulfilled_condition_missing_tag(self, validator):
        ...
        assert self.has_tag_error(result, 'Frame of Reference',
                                  '(0020,0052)', 'missing')
        assert self.has_tag_error(result, 'Synchronization',
                                  '(0018,106A)', 'missing')
    ...
    def test_condition_not_met_no_tag(self, validator):
        ...
        assert not self.has_tag_error(result, 'Frame of Reference',
                                      '(0020,0052)', 'missing')
        assert not self.has_tag_error(result, 'Frame of Reference',
    ...
    def test_and_condition_not_met(self, validator):
        ...
        assert not self.has_tag_error(result, 'Cardiac Synchronization',
                                      '(0018,1081)', 'missing')
        assert not self.has_tag_error(result, 'Cardiac Synchronization',
                                      '(0018,1082)', 'missing')
    ...
    def test_and_condition_met(self, validator):
        ...
        assert self.has_tag_error(result, 'Cardiac Synchronization',
                                  '(0018,1081)', 'missing')
        assert not self.has_tag_error(result, 'Cardiac Synchronization',
                                      '(0018,1082)', 'missing')
    ...
    def test_presence_condition_met(self, validator):
        ...
        assert self.has_tag_error(result, 'General Equipment',
                                  '(0028,0120)',
    ...
    def test_presence_condition_not_met(self, validator):
        ...
        assert not self.has_tag_error(result, 'General Equipment',
                                      '(0028,0120)',
                                      'missing')  # Pixel Padding Value
    ...
    def test_greater_condition_met(self, validator):
        ...
        assert self.has_tag_error(result, 'Image Pixel',
                                  '(0028,0006)',
    ...
    def test_greater_condition_not_met(self, validator):
        ...
        assert not self.has_tag_error(result, 'Image Pixel',
                                      '(0028,0006)',
                                      'missing')  # Planar configuration
    ...
    def test_condition_for_not_required_tag_cond2_fulfilled_not_present(self, validator):
        result = validator.validate()
