...
@pytest.fixture
def base_path(fs) -> Generator[Path, Any, None]:
...
def test_get_existing_edition(base_path):
    reader = MemoryEditionReader(
...
def test_non_existing_edition(base_path):
    reader = MemoryEditionReader(
        base_path, '<html><A HREF="/bla/">2014a</A>' '<a ref="foo">2014e</a>'
    )
...
def test_last_edition_in_year(base_path):
    reader = MemoryEditionReader(
        base_path,
        '<html><A HREF="/bla/">2014a</A>'
        '<a ref="foo">2014c</a>'
...
def test_current_edition(base_path):
    reader = MemoryEditionReader(
        base_path,
        '<html><A HREF="/bla/">2014a</A>'
        '<a ref="foo">2014c</a>'
...
def test_check_edition_existing(fs):
    base_path = Path("base")
    reader = MemoryEditionReader(base_path, "")
    fs.create_file(reader.editions_path(), contents='["2014a", "2014c", "2015a"]')
    edition, path = reader.get_edition_and_path("2014")
    assert edition == "2014c"
    assert path == base_path / "2014c"


def test_check_edition_nonexisting(fs):
    base_path = Path("/foo/bar")
    reader = MemoryEditionReader(base_path, "")
    fs.create_file(reader.editions_path(), contents='["2014a", "2014c", "2015a"]')
    edition, path = reader.get_edition_and_path("2016")
...
def test_is_current_version(fs, base_path):
    reader = MemoryEditionReader(base_path, "")
    assert not reader.is_current_version(CURRENT_EDITION)
    version_path = reader.version_path(CURRENT_EDITION)
    fs.create_file(version_path, contents="0.2.1")
    assert not reader.is_current_version(CURRENT_EDITION)
    version_path.unlink()
    fs.create_file(version_path, contents=__version__)
    assert reader.is_current_version(CURRENT_EDITION)


def test_write_current_version(fs, base_path):
    reader = MemoryEditionReader(base_path, "")
    assert not reader.is_current_version(CURRENT_EDITION)
    assert not reader.is_current_version(CURRENT_EDITION)
    reader.write_current_version(CURRENT_EDITION)
    assert reader.is_current_version(CURRENT_EDITION)
...
def test_recreate_json_if_needed(fs, base_path, edition_path):
    ...
    def create_json_files(self, edition):
        ...
        for name in (
            ...
            path = os.path.join(self.json_path(edition), name)
    ...
    reader = MemoryEditionReader(base_path, "")
    reader.create_json_files = lambda e: create_json_files(reader, e)
    fs.create_file(edition_path, contents='["2014a", "2014c", "2015a"]')
    current_edition = "2014a"
    assert reader.get_edition_path("2014e") is None
    assert reader.get_edition_path(current_edition) is not None
    assert create_json_files_called == 1
    reader.get_edition_path(current_edition)
    assert create_json_files_called == 2
    assert create_json_files_called == 2
    reader.write_current_version(current_edition)
    reader.get_edition_path(current_edition)
    assert create_json_files_called == 2
    reader.get_edition_path(current_edition, recreate_json=True)
...
def test_get_non_existing_edition(
