...
* tags not allowed in multi-frame functional groups have been listed
  as errors twice (see [#196](https://github.com/pydicom/dicom-validator/issues/196))
* fixed handling of enum values for a specific tag
* fixed handling of multiple definitions for referenced enum values
* fixed handling of conditions in functional groups (see [#226](https://github.com/pydicom/dicom-validator/issues/226))
...
### Fixes
* fixes to correctly evaluate SR documents (see [#206](https://github.com/pydicom/dicom-validator/issues/206)):
...
* some of the IOD-specific group macros (4 of 27) have not been found
  (see [#193](https://github.com/pydicom/dicom-validator/issues/193))
...
* fixed a regression bug introduced in 0.6.1, leading to an exception during docbook parsing
  (see [#119](https://github.com/pydicom/dicom-validator/issues/119))
...
* fixed checking of multi-valued tags with defined enum values (see [#87](https://github.com/pydicom/dicom-validator/issues/87))

## [Version 0.5.0](https://pypi.python.org/pypi/dicom-validator/0.5.0) (2024-01-25)
Adds enum checks and fixes a regression.

### Features
* added checking of most enumerated values (see [#54](https://github.com/pydicom/dicom-validator/issues/54))

### Fixes
* fixed a regression that causes an exception in the DICOM dump tool (see [#77](https://github.com/pydicom/dicom-validator/issues/77))
...
* added handling of conditional includes (needed for SR documents)
  (see [#39](https://github.com/pydicom/dicom-validator/issues/39))
...
* Condition parser: condition for AT values have not been correctly parsed if
  the condition used equality comparison (see [#58](https://github.com/pydicom/dicom-validator/issues/58))
...
### Features
* added support for validating functional group macros, see [#27](https://github.com/pydicom/dicom-validator/issues/27)
...
### Fixes
* fixed handling of unverifiable and condition, see [#32](https://github.com/pydicom/dicom-validator/issues/32)
* fixed too broad matching for "otherwise" condition, see [#29](https://github.com/pydicom/dicom-validator/issues/29)
* fixed too strict handling without "otherwise" condition, see [#38](https://github.com/pydicom/dicom-validator/issues/38)
...
* Condition parser: handle conditions without values,
  see [#15](https://github.com/pydicom/dicom-validator/issues/15)
* Condition parser: fixed handling of "zero" and "non-zero" values,
  see [#17](https://github.com/pydicom/dicom-validator/issues/17)
* Condition parser: handle a few more simple expressions
* Condition parser: ignore equality conditions with missing values
  (caused crash, see [#20](https://github.com/pydicom/dicom-validator/issues/20))
...
- fixed regression that broke the validator command line tool,
  see [#9](https://github.com/pydicom/dicom-validator/issues/9)
...
- all tags including PixelData are now loaded to account for dependencies on PixelData
  (see [#6](https://github.com/pydicom/dicom-validator/issues/6))
...
* account for added columns in IOD and UID tables (see [#3](https://github.com/pydicom/dicom-validator/issues/3))
